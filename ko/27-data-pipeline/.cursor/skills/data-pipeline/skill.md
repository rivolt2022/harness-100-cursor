...
| 에이전트 | 파일 | 역할 | 타입 |
|---------|------|------|------|
| etl-architect | `.cursor/agents/etl-architect.md` | 소스분석, 스키마설계, 파이프라인구조 | general-purpose |
| data-quality-manager | `.cursor/agents/data-quality-manager.md` | 검증규칙, 프로파일링, 이상탐지 | general-purpose |
| scheduler-engineer | `.cursor/agents/scheduler-engineer.md` | DAG설계, 의존관계, 재시도전략 | general-purpose |
| monitoring-specialist | `.cursor/agents/monitoring-specialist.md` | 메트릭, 알림, 대시보드, SLA | general-purpose |
| pipeline-reviewer | `.cursor/agents/pipeline-reviewer.md` | 교차검증, 정합성, 운영준비도 | general-purpose |
...
| 스킬 | 경로 | 강화 대상 에이전트 | 역할 |
|------|------|-----------------|------|
| data-quality-framework | `.cursor/skills/data-quality-framework/skill.md` | data-quality-manager | 품질 6차원, Great Expectations, dbt tests, 데이터 계약 |
| dag-orchestration-patterns | `.cursor/skills/dag-orchestration-patterns/skill.md` | scheduler-engineer | Airflow DAG 패턴, 멱등성, 재시도, 백필, 의존관계 |

...
| 에이전트 | 파일 | 역할 | 타입 |
|---------|------|------|------|
| vulnerability-scanner | `.cursor/agents/vulnerability-scanner.md` | CVE, 의존성, 설정오류 스캔 | general-purpose |
| code-analyst | `.cursor/agents/code-analyst.md` | SAST, 시큐어코딩, 패턴탐지 | general-purpose |
| pentest-reporter | `.cursor/agents/pentest-reporter.md` | 공격시나리오, PoC, 영향분석 | general-purpose |
| security-consultant | `.cursor/agents/security-consultant.md` | 개선권고, 로드맵, 프레임워크매핑 | general-purpose |
| audit-reviewer | `.cursor/agents/audit-reviewer.md` | 교차검증, 위험등급조정, 최종보고서 | general-purpose |
...
| 스킬 | 경로 | 강화 대상 에이전트 | 역할 |
|------|------|-----------------|------|
| owasp-testing-guide | `.cursor/skills/owasp-testing-guide/skill.md` | code-analyst, pentest-reporter | OWASP Top 10 취약점별 테스트 방법, 수정 가이드 |
| cve-analysis | `.cursor/skills/cve-analysis/skill.md` | vulnerability-scanner | CVSS 점수 해석, 의존성 스캔 도구, 시크릿 탐지 |
| threat-modeling | `.cursor/skills/threat-modeling/skill.md` | security-consultant, pentest-reporter | STRIDE, DREAD, Attack Tree, 공격 표면 분석 |

...
| 에이전트 | 파일 | 역할 | 타입 |
|---------|------|------|------|
| prompt-engineer | `.cursor/agents/prompt-engineer.md` | 프롬프트 설계 | general-purpose |
| rag-architect | `.cursor/agents/rag-architect.md` | RAG 파이프라인 | general-purpose |
| eval-specialist | `.cursor/agents/eval-specialist.md` | 평가 프레임워크 | general-purpose |
| optimization-engineer | `.cursor/agents/optimization-engineer.md` | 비용/성능 최적화 | general-purpose |
| deploy-engineer | `.cursor/agents/deploy-engineer.md` | 프로덕션 배포 | general-purpose |
...
1. 사용자 입력에서 추출한다:
    ...
    - **LLM 모델**: 사용할 모델 (기본: Cursor/GPT-4o)
...
| 메시지 기반 | Task 툴로 Subagent 호출 | 실시간 핵심 정보 전달, 피드백 |
...
| 스킬 | 파일 | 대상 에이전트 | 역할 |
|------|------|-------------|------|
| prompt-optimizer | `.cursor/skills/prompt-optimizer/skill.md` | prompt-engineer, eval-specialist | CRISP 루브릭, RCTF 템플릿, 가드레일 패턴, A/B 테스트, 토큰 최적화 |
| chunking-strategy-guide | `.cursor/skills/chunking-strategy-guide/skill.md` | rag-architect, eval-specialist | 청킹 전략 비교, 시맨틱 청킹 알고리즘, 문서별 전처리, 품질 메트릭 |

...
| 에이전트 | 파일 | 역할 | 타입 |
|---------|------|------|------|
| info-architect | `.cursor/agents/info-architect.md` | 구조 설계, 독자 분석, 목차 | general-purpose |
| doc-writer | `.cursor/agents/doc-writer.md` | 본문 작성, 코드 예제, 튜토리얼 | general-purpose |
| diagram-maker | `.cursor/agents/diagram-maker.md` | Mermaid 다이어그램, 시각 자료 | general-purpose |
| tech-reviewer | `.cursor/agents/tech-reviewer.md` | 정확성, 완전성, 일관성 검증 | general-purpose |
| version-controller | `.cursor/agents/version-controller.md` | 메타데이터, 변경 이력, 배포 | general-purpose |
...
| 확장 스킬 | 경로 | 대상 에이전트 | 역할 |
|----------|------|-------------|------|
| diagram-patterns | `.cursor/skills/diagram-patterns/skill.md` | diagram-maker | Mermaid 다이어그램 패턴 라이브러리 (아키텍처, 시퀀스, 플로차트, ER, 상태 다이어그램) |
| api-doc-standards | `.cursor/skills/api-doc-standards/skill.md` | doc-writer | REST API 문서 작성 표준 (엔드포인트 템플릿, 에러 코드, 페이지네이션, 인증) |
| code-example-patterns | `.cursor/skills/code-example-patterns/skill.md` | doc-writer | 코드 예제 패턴 (5단계 구조, 언어별 스타일, 튜토리얼 구성) |

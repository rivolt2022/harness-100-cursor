...
| 에이전트 | 파일 | 역할 | 타입 |
|---------|------|------|------|
| data-collector | `.cursor/agents/data-collector.md` | 데이터 수집·정제·정규화 | general-purpose |
| sentiment-analyst | `.cursor/agents/sentiment-analyst.md` | 감성분석, 감정 점수화 | general-purpose |
| topic-classifier | `.cursor/agents/topic-classifier.md` | 주제분류, 카테고리 설계 | general-purpose |
| trend-detector | `.cursor/agents/trend-detector.md` | 트렌드 분석, 이상 탐지 | general-purpose |
| insight-writer | `.cursor/agents/insight-writer.md` | 인사이트 보고서, 액션 아이템 | general-purpose |
...
| 확장 스킬 | 경로 | 대상 에이전트 | 역할 |
|----------|------|-------------|------|
| sentiment-scoring | `.cursor/skills/sentiment-scoring/skill.md` | sentiment-analyst | 감성 분류, 스코어링, NPS, 맥락 보정 |
| text-analytics-methods | `.cursor/skills/text-analytics-methods/skill.md` | topic-classifier, trend-detector | 주제 분류, 키워드 분석, 트렌드 탐지, 인사이트 도출 |
